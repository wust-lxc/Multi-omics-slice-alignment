...
def calcu_lisi(adata, domain_key, spatial_key):
    ...
    # 用 localconverter 替代 activate()
    with ro.conversion.localconverter(ro.default_converter + pandas2ri.converter):
        coords_r = ro.conversion.py2rpy(result[['x', 'y']])
        domain_r = ro.conversion.py2rpy(result[[domain_key]])
        res = lisi(coords_r, domain_r, ro.StrVector([domain_key]))
