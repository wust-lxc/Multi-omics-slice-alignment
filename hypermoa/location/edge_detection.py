...
def calcu_lisi(adata, domain_key, spatial_key):
    ...
    Input:
        ...
        domain_key: column name in .obs
        spatial_key: key in .obsm
    Output:
        DataFrame with LISI values
    '''
    result = adata.obs[[domain_key]].copy()
    result[['x', 'y']] = adata.obsm[spatial_key].copy()

    from sklearn.neighbors import NearestNeighbors

    coords = np.asarray(adata.obsm[spatial_key], dtype=np.float64)
    labels = result[domain_key].astype(str).to_numpy()
    ...
    neigh.fit(coords)
    neigh_index = neigh.kneighbors(coords, return_distance=False)[:, 1:]

    lisi_values = np.empty(n_obs, dtype=np.float64)
    for i, idx in enumerate(neigh_index):
        _, counts = np.unique(labels[idx], return_counts=True)
        probs = counts.astype(np.float64) / float(idx.shape[0])
        lisi_values[i] = 1.0 / np.sum(probs * probs)

    result['lisi'] = lisi_values
    return result
...
def detect_edge_of_domains(adatas, domain_key, domains_use, spatial_key='spatial', alpha=50):
    '''
    ...
    for i in range(len(domains_use)): 
        ...
        for domain_use_tmp in domain_use:
            ...
            # alpha-shape needs at least 4 points in each slice for this domain
            if (len(index_tmp1) < 4) or (len(index_tmp2) < 4):
                continue
            ...
            try:
                ...
                # Skip pathological domains and continue with available ones
                continue

            boundary_use_tmp1 += [index_tmp_dict1[idx] for idx in boundary_tmp1]
            boundary_use_tmp2 += [index_tmp_dict2[idx] for idx in boundary_tmp2]
            edge_use_tmp1 = edge_use_tmp1.union({(index_tmp_dict1[a], index_tmp_dict1[b]) for a, b in edge_tmp1})
            edge_use_tmp2 = edge_use_tmp2.union({(index_tmp_dict2[a], index_tmp_dict2[b]) for a, b in edge_tmp2})
