...
class Emb_Align(object):
    ...
    def prepare_hgat(self,
        ...
        if self.make_log:
            ...
            self.makeLog(f"  Feature key: {feat_key}")
            self.makeLog(f"  Neighbor number of intra-slice: {n_neigh_hom}")
            self.makeLog(f"  Neighbor number of inter-slice: {n_neigh_het}")
            self.makeLog(f"  Similarity cutoff of inter-slice: {c_neigh_het}")
            self.makeLog(f"  Adaptive Hyperedge Threshold: {sim_threshold}")
    ...
    def train_hgat( self,
        ...
        if mini_batch:
            # 这里的 mini-batch 逻辑原样保留，但注意超图模式下推荐使用 Full Batch (mini_batch=False)
            print("Warning: Mini-batch mode for Hypergraph not fully optimized, using default logic.")
            self.loaders = {}
            for key_ in self.slice_order:
                print(key_)
                num_ = self.data_hgat[key_].x.shape[0]
                per_batch = max(1, int(np.ceil(num_ / batches)))  
                seeds = make_seeds(num_, per_batch, batches)       
                loader = HGTLoader(
                                    self.data_hgat,
            ...
            for epoch in tqdm(range(epoch_hgat)):
                ...
                for batch_idx in range(batches):
                    ...
                    for key_ in batch_dict:
                        # 对于 Mini-batch, 需要从 batch 中提取超边 (这里可能会有形状不匹配风险，建议跑 Full batch)
                        # 如果 data_hgat 已经有了 hyperedge_index, loader 采样时会自动包含
                        # 但为保险起见，这里传入的 hyperedge_index 需要确认 Loader 是否正确处理了
                        hyperedge_index_dict = {nt: batch_dict[key_][nt].hyperedge_index for nt in batch_dict[key_].node_types}
                        
                        batch_out = self.hgat(
                            batch_dict[key_].x_dict.to(self.device),
                            batch_dict[key_].edge_index_dict.to(self.device),
                            hyperedge_index_dict=hyperedge_index_dict, # 传入超边
            ...
            # Full Batch Mode (Recommended for Hypergraph)
            loss_list = []
            for epoch in tqdm(range(epoch_hgat)):
                ...
                hyperedge_index_dict = {
                    node_type: self.data_hgat[node_type].hyperedge_index 
                    for node_type in self.data_hgat.node_types
                }

                out = self.hgat(x_dict=self.data_hgat.x_dict, 
                                edge_index_dict=self.data_hgat.edge_index_dict, 
                                hyperedge_index_dict=hyperedge_index_dict, # 新增
                                return_semantic_attention_weights=False)
    ...
    def predict_hgat(   self, 
        ...
        if mini_batch:
            # Mini-batch logic ... (Use with caution for Hypergraph)
            self.loaders_pred = {}
            for key_ in self.slice_order:
                num_ = self.data_hgat[key_].x.shape[0]
                per_batch = max(1, int(np.ceil(num_ / batches)))  
            ...
            for batch_tuple in zip(*self.loaders_pred.values()):
                ...
                for key_ in batch_dict.keys():
                    ...
                    batch_out, batch_atte = self.hgat(
                        batch_dict[key_].x_dict.to(self.device),
                        batch_dict[key_].edge_index_dict.to(self.device),
                        hyperedge_index_dict=hyperedge_index_dict,
            ...
            with torch.no_grad():
                ...
                if get_attention:
                    out, atte, atte_node = self.hgat(x_dict=self.data_hgat.x_dict,
                                                    edge_index_dict=self.data_hgat.edge_index_dict,
                                                    hyperedge_index_dict=hyperedge_index_dict, # 新增
                                                    return_semantic_attention_weights=True,
                                                    get_attention=True)
                else:
                    out, atte = self.hgat(x_dict=self.data_hgat.x_dict,
                                            edge_index_dict=self.data_hgat.edge_index_dict, 
                                            hyperedge_index_dict=hyperedge_index_dict, # 新增
                                            return_semantic_attention_weights=True)
