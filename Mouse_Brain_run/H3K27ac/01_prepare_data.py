...
def main():
    ...
    data_dir = resolve_data_root(root_dir) / "Mouse_Brain_H3K27ac"
    ...
    if not peak_file.exists():
        raise FileNotFoundError(f"Missing H3K27ac peak file: {peak_file}")
    ...
    if len(common_obs) == 0:
        raise ValueError("No overlapping barcodes between RNA and H3K27ac peak AnnData.")
    ...
    if "spatial" not in adata_rna.obsm:
        if "spatial" not in adata_peak.obsm:
            raise KeyError("No spatial coordinates found in RNA or H3K27ac AnnData.")
    ...
    adata_rna.obs["batch"] = "H3K27ac"
    adata_rna.obs["slice_order"] = 0
    adata_rna.obs["epigenomic_assay"] = "H3K27ac"
    if "H3K27ac_clusters" in adata_rna.obs:
        adata_rna.obs["Domain_input"] = adata_rna.obs["H3K27ac_clusters"].astype(str)
    ...
    print(f"Prepared H3K27ac single-slice data: cells={adata_rna.n_obs}, genes={adata_rna.n_vars}")
